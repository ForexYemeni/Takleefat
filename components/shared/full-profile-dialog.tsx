...
export interface FullProfile {
  ...
  createdAt: string
  /** الجولة 61: رابط صورة البروفايل — يظهر لكل المشاهدات (اختيار صاحبها) */
...
export function FullProfileDialog({
  ...
  // الجولة 61: المستندات تُفتح بمنح إداري صريح — وحالة الطلب تظهر في لوحة الخصوصية
  ...
  const documentAccess = data?.documentAccess
            ...
            <div className="grid gap-4 lg:grid-cols-5">
              ...
              <div className="space-y-4 lg:col-span-3">
                ...
                <Section
                  title={
                    documentsHidden
                      ? 'المستندات الرسمية — محمية بإذن الإدارة'
                      : `المستندات المرفوعة (${documents.length})`
                  }
                  icon={documentsHidden ? ShieldCheck : FileText}
                >
                  {documentsHidden ? (
                    // الجولة 61 — السياسة الجديدة: لوحة الخصوصية توضّح السياسة،
                    // تعرض شارات الجاهزية، وتتيح «طلب رؤية المستندات» بسبب معلن
                    // (تُلغي لوحة الجولة 45 الثابتة — قرار صاحب المنصة للشفافية)
                    <DocumentAccessPanel
                      targetId={profile.id}
...
async function apiFetchProfile(userId: string): Promise<{
  ...
  /** الجولة 61: حالة طلب رؤية المستندات للمشاهد الحالي */
