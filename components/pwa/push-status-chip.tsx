  ...
  const testNow = async () => {
    ...
    try {
      ...
      if (test.delivered > 0) {
        toast.success(
          test.delivered === 1
            ? 'أُرسل إشعار تجريبي إلى جهازك — إن لم يظهر فافحص إعدادات إشعارات الموقع في المتصفح'
            : `أُرسل إشعار تجريبي إلى ${test.delivered} أجهزة — إن لم يظهر فافحص إعدادات إشعارات الموقع`
        )
      } else {
        setState('off')
