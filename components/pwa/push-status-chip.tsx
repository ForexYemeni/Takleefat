    ...
    try {
      ...
      if (result === 'on') {
        ...
        if (test && test.delivered > 0) {
          toast.success('أرسلنا إشعاراً تجريبياً — كل الإشعارات الحقيقية ستصل بنفس الطريقة')
        }
        return
      }
      if (result === 'denied') {
  ...
  const testNow = async () => {
    ...
    try {
      ...
      if (test.delivered > 0) {
        toast.success(
          test.delivered === 1
            ? 'أُرسل إشعار تجريبي إلى جهازك بنجاح — هكذا ستصل كل الإشعارات الحقيقية صوتاً وتنبيهاً حتى والتطبيق مغلق. إن لم يظهر خلال دقيقة: فعّل إشعارات المتصفح/التطبيق من إعدادات النظام وعطّل مُحسِّن البطارية ووضع عدم الإزعاج'
            : `أُرسل إشعار تجريبي إلى ${test.delivered} أجهزة — هكذا ستصل كل الإشعارات الحقيقية حتى والتطبيق مغلق. إن لم يظهر على جهاز معين ففعّل إشعارات المتصفح من إعدادات النظام`
        )
      } else {
        setState('off')
