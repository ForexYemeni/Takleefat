 ...
 * - يظهر الكادر المضاف فوراً في حساب الإدارة (الكادر الصحي) وفي لوحة الجهة
 * - لا يستقبل أي تكليف أو إجراء قبل اعتماده من الإدارة ورفع مستنداته
 *
 * الجولة 39 — اعتماد الجهة + الاعتماد المهني:
 * - المستلم الإداري يعتمد الكادر الصحي لجهته (إضافتهم لمجتمع كوادر الجهة فقط)
...
interface StaffNurse {
  ...
  /** الجولة 40: طلب انضمام ذاتي من الكادر (PENDING وقد طلبه بنفسه) */
  isJoinRequest?: boolean
  /** الجولة 40: نوع العمل الذي طلبه الكادر (يعمل حالياً/عمل سابقاً) */
  requestedStatusLabel?: string | null
  /** الجولة 40: ملاحظة الكادر مع طلبه */
  joinNote?: string | null
  nurse: {
    id: string
    name: string
    /** الجولة 39: دور العضو — المستلم يعتمد الكادر الصحي حصراً */
    role: string
...
export default function ReceiverStaffPage() {
  ...
  // الجولة 42: كتالوج الأقسام من حساب الإدارة — القسم من الكتالوج لا كتابة حرة
  const [deptOptions, setDeptOptions] = useState<Array<{ id: string; name: string }>>([])

  useEffect(() => {
    fetch('/api/departments/public')
      .then((r) => r.json())
      .then((d) => setDeptOptions(d.departments ?? []))
      .catch(() => null)
  }, [])
  ...
  // ---------- الجولة 42: قبول طلب الانضمام بحالة يختارها مسؤول الجهة ----------
  ...
  // ---------- الجولة 40: رفض طلب الانضمام (إزالة الطلب مع إشعار الكادر) ----------
  ...
  // الجولة 40: فصل طلبات الانضمام الذاتية عن صفوف الكوادر — قرار قبول/رفض صريح
  // بدل أن يختلط الطلب بالقائمة ويبدو خللاً في القسم — المستلم يعتمد الكادر
  // التمريضي حصراً، وطلبات الأطباء من اختصاص مشرف الأطباء (الجولة 39)
  const joinRequests = nurses.filter((n) => n.isJoinRequest && n.nurse.role === 'NURSE')
  const staffRows = nurses.filter((n) => !(n.isJoinRequest && n.nurse.role === 'NURSE'))
  ...
  return (
    <div className="space-y-4">
      ...
      {/* ---------- الجولة 40: طلبات الانضمام إلى جهتك — قرار قبول/رفض صريح ---------- */}
        ...
        <section className="overflow-hidden rounded-3xl border border-sky-200 bg-sky-50/60 dark:border-sky-900/60 dark:bg-sky-950/30">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-sky-200 bg-sky-100/60 px-4 py-3 dark:border-sky-900/60 dark:bg-sky-900/30">
            <h2 className="flex items-center gap-2 text-sm font-extrabold text-sky-900 dark:text-sky-200">
              ...
              طلبات انضمام إلى جهتك
              <Badge className="bg-sky-600 text-white">{joinRequests.length}</Badge>
            </h2>
            <p className="text-[11px] text-muted-foreground">
              كوادر تمريضيون يطلبون إضافة أنفسهم لجهتك — القبول يضيفهم لمجتمع كوادر الجهة فقط،
      ...
      {view === 'all' ? (
        ...
        <EmptyState
          icon={Users}
          title={joinRequests.length > 0 ? 'لا يوجد كوادر معتمدين بعد — راجع طلبات الانضمام أعلاه' : 'لا يوجد كوادر في جهتك بعد'}
      ...
      <Dialog open={createOpen} onOpenChange={setCreateOpen}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          ...
          >
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label>القسم — من كتالوج الإدارة *</Label>
                {/* الجولة 42: منتقي القسم من كتالوج الأقسام المُدار من حساب الإدارة
                    — لا كتابة حرة (نفس قاعدة كتالوج التخصصات للأطباء) */}
                <Select
                  ...
                  <SelectContent>
                    {deptOptions.length === 0 ? (
                      <div className="px-3 py-2 text-xs text-muted-foreground">
                        لا توجد أقسام في الكتالوج — راجع الإدارة
                      </div>
                    ) : (
                      deptOptions.map((d) => (
        ...
        <DialogContent>
          ...
          {details && (
            <div className="space-y-3">
              ...
              {/* الجولة 40: تفاصيل طلب الانضمام — ملاحظة الكادر ونوع العمل المطلوب */}
              {details.isJoinRequest && (
                <div className="space-y-2 rounded-xl border border-sky-200 bg-sky-50 p-3 text-xs leading-relaxed text-sky-900 dark:border-sky-900 dark:bg-sky-950/40 dark:text-sky-200">
                  <p className="flex items-start gap-2">
                    ...
                    هذا <span className="font-bold">طلب انضمام ذاتي</span> قدّمه الكادر بنفسه —
                  ...
                  {details.joinNote && <p>ملاحظة الكادر: «{details.joinNote}»</p>}
                </div>
              )}
      ...
      <Dialog open={!!statusEditing} onOpenChange={(open) => !open && setStatusEditing(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              حوّل حالة الكادر في جهة ({org?.name}) بين الحالات المهنية الخمس في أي وقت —
              الحالة الحالية معلَّمة، والحالات الإدارية تبقى من حساب الإدارة حصراً.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-2" role="radiogroup" aria-label="تحويل حالة الكادر في الجهة">
      ...
      {/* ---------- الجولة 42: قبول طلب الانضمام مع اختيار حالة الكادر في الجهة ---------- */}
      <Dialog open={!!accepting} onOpenChange={(open) => !open && setAccepting(null)}>
        <DialogContent className="max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              اختر حالة الكادر في جهة ({org?.name}) عند قبوله — كل الخيارات تضيفه لكوادر الجهة
              بحسب حالته، واعتماده المهني يبقى من حساب الإدارة بعد المستندات.
            </DialogDescription>
          </DialogHeader>
          <div className="grid gap-2" role="radiogroup" aria-label="حالة الكادر في الجهة">
            {ORG_CADRE_STATUS_OPTIONS.map((opt) => {
      ...
      {/* ---------- الجولة 40: تأكيد رفض طلب الانضمام ---------- */}
      <ConfirmDialog
        open={!!rejecting}
