...
function NurseAssignmentsContent() {
  ...
  // الجولة 49: نوافذ وقت تكليفات الكادر السارية — لتحذير التقديم على وقت متعارض
  // (بلاغ: «لا يتمكن الكادر من التقديم في تكليف جديد اذا كان بنفس التاريخ والوقت الذي هو يعمل فية»)
...
function AvailablePosts({
  ...
  /** الجولة 49: نوافذ وقت التكليفات السارية للكادر — لمنع التقديم على وقت متعارض */
      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid gap-4 md:grid-cols-2">
          {filtered.map((post) => {
            ...
            // الجولة 49: هل يتقاطع وقت هذا التكليف مع تكليف سارٍ للكادر؟
                ...
                <CardContent className="space-y-3 p-5">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <Link href={`/nurse/assignments/${post.id}`} className="font-bold hover:text-primary hover:underline focus-visible:outline-2 focus-visible:outline-[var(--role-accent)]">
                        {post.title}
                      </Link>
...
function NurseAssignmentCard({
  ...
  // انتهاء الوقت 100%» — زر التأكيد يظهر بعد بلوغ وقت الانتهاء حصراً
  // (التكليف بلا وقت انتهاء محدد يبقى كما كان)
    ...
    <div className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-amber-200 bg-amber-50/70 px-3 py-2.5 dark:border-amber-900 dark:bg-amber-950/30">
      <div className="min-w-0">
        ...
        <p className="mt-0.5 text-[11px] leading-relaxed text-muted-foreground">
          ...
          عند وجود ظرف طارئ يُنهيه المستلم الإداري من جهته مع ذكر السبب
        </p>
      </div>
