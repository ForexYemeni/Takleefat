      ...
      <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-center">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            أعلن عن تكليفاتك، راجع سير الطبيب المتقدم، واعتمد الأنسب — أو تأكّد من التكليفات المؤكدة
          </p>
        </div>
  ...
  if (posts.length === 0) {
    return (
      <EmptyState
        ...
        description="أنشئ أول تكليف ليظهر للطبيب التمريضي ويقدّم عليه — ثم راجع سيرهم الذاتية واعتمد الأنسب."
      />
    )
  }
            ...
            <CardContent className="space-y-3 p-5">
              ...
              <div className="flex flex-wrap gap-2 text-xs">
                ...
                <Badge variant="outline" className="gap-1">
                  ...
                  {approvedCount} / {post.nursesNeeded} طبيب معتمد
                </Badge>
                {post.hours ? (
                  <Badge variant="outline" className="gap-1">
              ...
              {post.status === 'OPEN' ? (
                ...
                <p className="flex items-center gap-1.5 rounded-xl bg-cyan-50 px-3 py-2 text-xs font-bold text-cyan-700">
                  ...
                  اكتمل اختيار الطبيب لهذا التكليف — تابع التكليف المؤكد من تبويب «التكليفات المؤكدة»
                </p>
              )}
  ...
  if (applications.length === 0) {
    return (
      <p className="rounded-xl border border-dashed p-8 text-center text-sm text-muted-foreground">
        لا توجد تقديمات بعد — سيصلك إشعار فور تقديم أي طبيب على هذا التكليف
      </p>
    )
  }

  // ---------- عرض السيرة الذاتية الاحترافية — عند الضغط على بطاقة الطبيب ----------
  if (selected) {
    return (
    ...
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <p className="rounded-xl bg-secondary/60 px-3 py-2 text-xs text-muted-foreground">
          المطلوب: {nursesNeeded} طبيب — عند اكتمال العدد يُغلق التكليف تلقائياً وتُرفض بقية التقديمات
        </p>
      ...
      <p className="text-xs text-muted-foreground">
        اضغط على بطاقة الطبيب لعرض سيرته الذاتية كاملة — البيانات والمستندات والتقييمات
      </p>
...
// ---------- مراسلة الطبيب عبر واتساب (الجولة العاشرة) ----------
...
function ConfirmedAssignments({ assignments }: { assignments: ReceiverAssignment[] }) {
  ...
  if (assignments.length === 0) {
    return (
      <EmptyState
        ...
        description="تظهر هنا التكليفات المؤكدة بعد اعتماد تقديمات الطبيب أو الإسناد المباشر من الإدارة."
      />
    )
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {/* تكليفات جارية — تم اختيار الطبيب */}
      {working.length > 0 && (
        <div className="space-y-3">
                ...
                <CardContent className="space-y-3 p-5">
                  ...
                  {a.nurseDoneAt ? (
                    <p className="flex items-center gap-1.5 rounded-xl bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700 dark:bg-emerald-950/40 dark:text-emerald-300">
                      <BadgeCheck className="size-3.5" />
                      أكد الطبيب انتهاء التكليف واستلام المبلغ — {formatDateTime(a.nurseDoneAt)}
                    </p>
                  ) : (
                    <p className="rounded-xl bg-secondary/60 px-3 py-2 text-xs text-muted-foreground">
                      الطبيب يعمل على التكليف حالياً — عند الانتهاء اضغط «تم انتهاء التكليف» لتأكيد
                      الدفع وتقييم الطبيب
                    </p>
                  )}
                  ...
                  {a.rating && (
                    <div className="rounded-xl border bg-secondary/40 p-3">
                      <p className="flex items-center justify-between gap-2 text-xs">
                        <span className="font-bold">تقييمك للطبيب</span>
    ...
    <Dialog open={!!assignment} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="max-h-[90vh] max-w-lg overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            خطوتان أخيرتان: تأكيد الدفع للطبيب، ثم تقييمه بشكل احترافي — يظهر تقييمك في ملف
            الطبيب ويُضاف إلى سيرته الذاتية عند التقديم لأي تكليف آخر
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-2xl border bg-gradient-to-bl from-amber-50 to-transparent p-4 dark:from-amber-950/20">
            <p className="flex items-center gap-2 text-sm font-extrabold">
              ...
              تقييم الطبيب — {assignment.nurse.name}
            </p>
            ...
            <div className="mt-3 space-y-3">
              ...
              <div className="space-y-1.5">
                <Label htmlFor="rating-comment">تعليق على أداء الطبيب (يظهر في سيرته الذاتية)</Label>
                <Textarea
                  id="rating-comment"
                  rows={3}
