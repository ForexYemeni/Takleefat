 ...
 * - RECEIVER: ارتباطات جهته الصحية فقط — كادر صحي حصراً (الجولة 39)
 ...
 *  - اعتماد الجهة (إضافة لمجتمع كوادر الجهة): يمنحه المستلم الإداري للكادر الصحيي
...
async function loadWithAccess(id: string, role: string, userId: string) {
  ...
  if (role === 'RECEIVER' || role === 'DOCTOR_SUPERVISOR') {
    ...
    // مطابقة الدور مع الهدف: المستلم → كادر صحي | مشرف الأطباء → طبيب
    const expectedRole = role === 'DOCTOR_SUPERVISOR' ? 'DOCTOR' : 'NURSE'
    if (affiliation.nurse.role !== expectedRole) {
      return {
        error: jsonError(
          role === 'DOCTOR_SUPERVISOR'
            ? 'اعتماد الأطباء من اختصاص مشرف الأطباء — والكادر الصحي من اختصاص المستلم الإداري'
            : 'اعتماد الكادر الصحي من اختصاص المستلم الإداري — والأطباء من اختصاص مشرف الأطباء',
          403
        ),
      }
    }
