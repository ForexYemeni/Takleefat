 ...
 * المستلم الإداري → يضيف كادر صحي لجهته | مشرف الأطباء → يضيف أطباء
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (isSupervisor) {
      ...
      // الجولة 42: القسم/التخصص للكادر الصحيي إجباري من كتالوج الأقسام المُدار
    ...
    const nurse = await db.$transaction(async (tx) => {
      const user = await tx.user.create({
        data: {
          ...
          phone,
          password: hashed,
          // مشرف الأطباء ينشئ أطباء — المستلم ينشئ كادر صحي (منظومة الأطباء)
      ...
      admins.map((a) =>
        notify(a.id, {
          title: isSupervisor
            ...
            : 'كادر صحي جديد أضافه المستلم الإداري',
          body: org
