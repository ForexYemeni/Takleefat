 ...
 * - الكادر الصحي والطبيب: بياناته + مستنداته + تكليفاته + تقييماته
...
export async function GET(
  ...
  try {
    ...
    // ---------- الكادر الصحي: مستندات + تكليفات + تقييمات ----------
...
export async function PATCH(
  ...
  try {
    ...
    // ⚠️ ترتيب حاسم: هذا الفرع يجب أن يسبق فرع «إعادة تعيين كلمة المرور» —
    // لأن طلب النقل يرسل { role, password } حيث password هي كلمة تأكيد الإدارة
    // وليست كلمة مرور جديدة للحساب — لو وصل الطلب لفرع كلمة المرور لأُعيدت
    // تعيين كلمة مرور الحساب بكلمة تأكيد الإدارة دون نقل (حادثة الجولة 59 المصححة).
    // البلاغ الحرفي: «اتمكن من نقل اي كادر صحي الى مستلم اداري او مشرف اطباء
    ...
    if (body && typeof body === 'object' && 'trustedContactViewer' in body) {
      ...
      await notify(id, {
        ...
        body: trustedContactViewer
          ? 'منحتك إدارة المنصة إذن رؤية بيانات اتصال أي كادر صحي أو طبيب في أي وقت — حتى بعد إنهاء التكليفات.'
    ...
    // سياسة الاعتماد (الجولة 34): لا يُعتمد أي كادر صحي أو طبيب قبل رفع مستنداته —
    // بلا مستندات لا توجد موافقة — دون استثناء (يشمل من أضافهم المستلم/المشرف ومن سجّل ذاتياً)
    if (status === 'APPROVED' && (target.role === 'NURSE' || target.role === 'DOCTOR')) {
      const documentsCount = await db.document.count({ where: { userId: id } })
      if (documentsCount === 0) {
        return jsonError(
          target.role === 'DOCTOR'
            ? 'لا يمكن اعتماد الطبيب قبل رفع مستنداته (الهوية وصورة المزاولة) — رفع المستندات إجباري دون استثناء — اطلب منه رفع المستندات أولاً'
            : 'لا يمكن اعتماد الكادر الصحي قبل رفع مستنداته (الهوية وصورة المزاولة) — اطلب منه رفع المستندات أولاً',
          422
        )
      }
    }
 ...
 * كادر صحي يجب ان تظهر تاكيد بكلمة المرور الخاصة بحساب الادارة»:
