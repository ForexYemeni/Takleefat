 ...
 * تكليفات المستخدم الحالي (الكادر الصحي أو المستلم الإداري أو مشرف الأطباء)
 ...
 * الجولة 38 — القفل باتجاه واحد (أُلغيت — الجولة 48):
 *  - كانت بيانات اتصال المستلم الإداري ومشرف الأطباء لا تُرسل للكادر/الطبيب
 *    أبداً تحت أي ظرف — حتى بعد تأكيد الدفع، وهو البلاغ الحرفي:
 *    «في تقديماتي يبقى رقم التواصل مغلق رغم انه تم تاكيد الدفع».
 * الجولة 48 — الاتصال المتناظر (القاعدة الحالية):
 *  - بيانات اتصال المستلم/المشرف تُفتح للكادر بالقاعدة الموحدة نفسها
 *    (isAssignmentContactOpen): بعد تأكيد الإدارة سداد الرسوم، أو بلا أي
 *    رسوم (عرض بدون رسوم/إسناد مباشر) — أثناء سير التكليف حصراً، وتُغلق
 *    تلقائياً فور الإنهاء/الإلغاء — تناظر كامل مع اتجاه الكادر→المستلم.
...
export async function GET() {
  try {
    ...
    const shaped = assignments.map((a) => ({
      ...
      receiver: {
        ...a.receiver,
        // الجولة 48: الاتصال المتناظر — القاعدة الموحدة isAssignmentContactOpen
        // في الاتجاهين: يُفتح رقم المستلم/المشرف للكادر بعد تأكيد الإدارة
        // سداد الرسوم (أو بلا أي رسوم) أثناء سير التكليف، ويُغلق تلقائياً
        // بعد الإنهاء/الإلغاء. والمستلم يرى رقم نفسه دائماً.
        ...(isWorker
          ? receiverPhoneForStaff(
              a.receiver.phone,
              isAssignmentContactOpen(a, applicationFee)
