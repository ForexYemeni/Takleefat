    ...
    <div className="space-y-4">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          بيانات حساب مدير النظام وإدارة إعدادات الدخول
        </p>
      </div>

      {/* بطاقة الهوية */}
      <Card className="overflow-hidden">
        <div className="h-20 bg-gradient-to-l from-slate-800 via-teal-700 to-teal-500" />
        <CardContent className="relative -mt-10 pb-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
            <Avatar className="size-20 border-4 border-background shadow-lg">
              <AvatarFallback className="bg-slate-900 text-xl font-extrabold text-teal-300">
                {initials || '؟'}
              </AvatarFallback>
            </Avatar>
          ...
          {user && (
            <div className="mt-6 grid grid-cols-4 gap-3">
              <Stat icon={UserRoundCheck} label="كادر معتمد مُدبَر" value={user._count.assignments} />
              <Stat icon={ClipboardList} label="تكليف مُعلن" value={user._count.posts} />
              <Stat icon={UserRoundX} label="تقديم مُراجَع" value={user._count.applications} />
              <Stat icon={FileCheck2} label="مستند مُراجَع" value={user._count.documents} />
            </div>
          )}
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>الاسم الظاهر في المنصة</CardDescription>
        </CardHeader>
        <CardContent>
        ...
        <CardContent>
          <form onSubmit={savePassword} className="grid gap-4 sm:grid-cols-3" noValidate>
            <div className="space-y-2">
              <Label htmlFor="ap-current">كلمة المرور الحالية</Label>
              <Input
                id="ap-current"
            ...
            <div className="space-y-2">
              <Label htmlFor="ap-new">كلمة المرور الجديدة</Label>
              <Input
                id="ap-new"
                type={showPasswords ? 'text' : 'password'}
            ...
            <div className="space-y-2">
              <Label htmlFor="ap-confirm">تأكيد كلمة المرور</Label>
              <Input
                id="ap-confirm"
                type={showPasswords ? 'text' : 'password'}
                autoComplete="new-password"
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            الرقم هو معرّف الدخول لحساب الإدارة — بعد التغيير استخدم الرقم الجديد في تسجيل الدخول
          </CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={savePhone} className="grid gap-4 sm:grid-cols-3" noValidate>
            <div className="space-y-2">
              <Label htmlFor="aph-current">كلمة المرور الحالية</Label>
              <Input
                id="aph-current"
                type={showPasswords ? 'text' : 'password'}
                autoComplete="current-password"
            ...
            <div className="space-y-2">
              <Label htmlFor="aph-new">الرقم الجديد</Label>
              <Input
                id="aph-new"
                dir="ltr"
                inputMode="tel"
