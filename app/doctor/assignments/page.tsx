...
function AvailablePosts({
  ...
  // تخصصات عمل الطبيب — لإبراز التكليفات المطابقة لتخصصاته بشارة فاخرة
  const { data: wdData } = useQuery({
    queryKey: ['my-work-specialties'],
    queryFn: () =>
      apiFetcher<{ departments: Array<{ id: string; name: string }> }>('/api/me/work-specialties'),
    staleTime: 60_000,
  })
      ...
      <CardContent className="space-y-2.5 p-4">
        ...
        {showDetails && (
          <div className="space-y-3">
            {/* بطاقة طرق الدفع للإدارة — للطبيب فقط */}
            {hasFinanceSection && (
              <PaymentCard
                settings={settings!}
